...
class PlayerBiddingStrategy(DtBiddingStrategy):
    def __init__(self, budget=100, name="Decision-Transformer-PlayerStrategy", cpa=2, category=1):
        ...
        self.model = DecisionTransformer(state_dim=16, act_dim=1, state_mean=normalize_dict["state_mean"], state_std=normalize_dict["state_std"])
    ...
    def mean_of_last_n_elements(history, n):
        ...
        alpha = self.model.take_actions(test_state, pre_reward=sum(history_conversion[-1]) if len(history_conversion) != 0 else None)
        bids = alpha * pValues
        return bids
